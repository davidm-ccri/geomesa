...
import geomesa.core.iterators.AttributeIndexFilteringIterator
...
class BatchMultiScannerTest extends Specification {
  ...
  val sftName = "bmstest"
  val sft = DataUtilities.createType(sftName, s"name:String,age:String,idStr:String,dtg:Date,*geom:Geometry:srid=4326")
  ...
  def attrIdxEqualQuery(attr: String, value: String, batchSize: Int): Int = {
    ...
    conn.tableOperations().exists(attrIdxTable) mustEqual true
    ...
    attrScanner.addScanIterator(is)
    ...
    conn.tableOperations().exists(recordTable) mustEqual true
